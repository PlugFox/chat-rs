...
class FrameHeader {
  const FrameHeader({required this.kind, required this.seq, required this.eventSeq});
...
class Frame {
  const Frame({required this.seq, required this.eventSeq, required this.payload});
  final int seq;
  final int eventSeq;
...
void encodeFrame(ProtocolWriter w, Frame frame) {
  ...
  switch (frame.payload) {
    case FramePayloadHello p: encodeHelloPayload(w, p.data);
    case FramePayloadWelcome p: encodeWelcomePayload(w, p.data);
    case FramePayloadPing(): break;
    case FramePayloadPong(): break;
    case FramePayloadRefreshToken p: encodeRefreshTokenPayload(w, p.data);
    case FramePayloadSendMessage p: encodeSendMessagePayload(w, p.data);
    case FramePayloadEditMessage p: encodeEditMessagePayload(w, p.data);
    case FramePayloadDeleteMessage p: encodeDeleteMessagePayload(w, p.data);
    case FramePayloadReadReceipt p: encodeReadReceiptPayload(w, p.data);
    case FramePayloadTyping p: encodeTypingPayload(w, p.data);
    case FramePayloadGetPresence p: encodeGetPresencePayload(w, p.data);
    case FramePayloadLoadChats p: encodeLoadChatsPayload(w, p.data);
    case FramePayloadSearch p: encodeSearchPayload(w, p.data);
    case FramePayloadSubscribe p: encodeSubscribePayload(w, p.data);
    case FramePayloadUnsubscribe p: encodeUnsubscribePayload(w, p.data);
    case FramePayloadLoadMessages p: encodeLoadMessagesPayload(w, p.data);
    case FramePayloadAddReaction p: encodeAddReactionPayload(w, p.data);
    case FramePayloadRemoveReaction p: encodeRemoveReactionPayload(w, p.data);
    case FramePayloadPinMessage p: encodePinMessagePayload(w, p.data);
    case FramePayloadUnpinMessage p: encodeUnpinMessagePayload(w, p.data);
    case FramePayloadForwardMessage p: encodeForwardMessagePayload(w, p.data);
    case FramePayloadMessageNew p: encodeMessage(w, p.data);
    case FramePayloadMessageEdited p: encodeMessage(w, p.data);
    case FramePayloadMessageDeleted p: encodeMessageDeletedPayload(w, p.data);
    case FramePayloadReceiptUpdate p: encodeReceiptUpdatePayload(w, p.data);
    case FramePayloadTypingUpdate p: encodeTypingUpdatePayload(w, p.data);
    case FramePayloadMemberJoined p: encodeMemberJoinedPayload(w, p.data);
    case FramePayloadMemberLeft p: encodeMemberLeftPayload(w, p.data);
    case FramePayloadPresenceResult p: w.writeU16(p.data.length); for (final e in p.data) { encodePresenceEntry(w, e); }
    case FramePayloadChatUpdated p: encodeChatEntry(w, p.data);
    case FramePayloadChatCreated p: encodeChatEntry(w, p.data);
    case FramePayloadReactionUpdate p: encodeReactionUpdatePayload(w, p.data);
    case FramePayloadUserUpdated p: encodeUserEntry(w, p.data);
    case FramePayloadChatDeleted p: encodeChatDeletedPayload(w, p.data);
    case FramePayloadMemberUpdated p: encodeMemberUpdatedPayload(w, p.data);
    case FramePayloadAck p: w.writeRawBytes(p.data);
    case FramePayloadError p: encodeErrorPayload(w, p.data);
    case FramePayloadCreateChat p: encodeCreateChatPayload(w, p.data);
    case FramePayloadUpdateChat p: encodeUpdateChatPayload(w, p.data);
    case FramePayloadDeleteChat p: encodeDeleteChatPayload(w, p.data);
    case FramePayloadGetChatInfo p: encodeGetChatInfoPayload(w, p.data);
    case FramePayloadGetChatMembers p: encodeGetChatMembersPayload(w, p.data);
    case FramePayloadInviteMembers p: encodeInviteMembersPayload(w, p.data);
    case FramePayloadUpdateMember p: encodeUpdateMemberPayload(w, p.data);
    case FramePayloadLeaveChat p: encodeLeaveChatPayload(w, p.data);
    case FramePayloadMuteChat p: encodeMuteChatPayload(w, p.data);
    case FramePayloadUnmuteChat p: encodeUnmuteChatPayload(w, p.data);
    case FramePayloadGetUser p: encodeGetUserPayload(w, p.data);
    case FramePayloadGetUsers p: encodeGetUsersPayload(w, p.data);
    case FramePayloadUpdateProfile p: encodeUpdateProfilePayload(w, p.data);
    case FramePayloadBlockUser p: encodeBlockUserPayload(w, p.data);
    case FramePayloadUnblockUser p: encodeUnblockUserPayload(w, p.data);
    case FramePayloadGetBlockList p: encodeGetBlockListPayload(w, p.data);
  }
}
...
Frame decodeFrame(ProtocolReader r) {
  ...
  switch (header.kind.value) {
    case 1: payload = FramePayloadHello(decodeHelloPayload(r));
    case 2: payload = FramePayloadWelcome(decodeWelcomePayload(r));
    case 3: payload = FramePayloadPing();
    case 4: payload = FramePayloadPong();
    case 5: payload = FramePayloadRefreshToken(decodeRefreshTokenPayload(r));
    case 16: payload = FramePayloadSendMessage(decodeSendMessagePayload(r));
    case 17: payload = FramePayloadEditMessage(decodeEditMessagePayload(r));
    case 18: payload = FramePayloadDeleteMessage(decodeDeleteMessagePayload(r));
    case 19: payload = FramePayloadReadReceipt(decodeReadReceiptPayload(r));
    case 20: payload = FramePayloadTyping(decodeTypingPayload(r));
    case 21: payload = FramePayloadGetPresence(decodeGetPresencePayload(r));
    case 22: payload = FramePayloadLoadChats(decodeLoadChatsPayload(r));
    case 23: payload = FramePayloadSearch(decodeSearchPayload(r));
    case 24: payload = FramePayloadSubscribe(decodeSubscribePayload(r));
    case 25: payload = FramePayloadUnsubscribe(decodeUnsubscribePayload(r));
    case 26: payload = FramePayloadLoadMessages(decodeLoadMessagesPayload(r));
    case 27: payload = FramePayloadAddReaction(decodeAddReactionPayload(r));
    case 28: payload = FramePayloadRemoveReaction(decodeRemoveReactionPayload(r));
    case 29: payload = FramePayloadPinMessage(decodePinMessagePayload(r));
    case 30: payload = FramePayloadUnpinMessage(decodeUnpinMessagePayload(r));
    case 31: payload = FramePayloadForwardMessage(decodeForwardMessagePayload(r));
    case 32: payload = FramePayloadMessageNew(decodeMessage(r));
    case 33: payload = FramePayloadMessageEdited(decodeMessage(r));
    case 34: payload = FramePayloadMessageDeleted(decodeMessageDeletedPayload(r));
    case 35: payload = FramePayloadReceiptUpdate(decodeReceiptUpdatePayload(r));
    case 36: payload = FramePayloadTypingUpdate(decodeTypingUpdatePayload(r));
    case 37: payload = FramePayloadMemberJoined(decodeMemberJoinedPayload(r));
    case 38: payload = FramePayloadMemberLeft(decodeMemberLeftPayload(r));
    case 39: payload = FramePayloadPresenceResult(r.readArray(r.readU16(), () => decodePresenceEntry(r)));
    case 40: payload = FramePayloadChatUpdated(decodeChatEntry(r));
    case 41: payload = FramePayloadChatCreated(decodeChatEntry(r));
    case 42: payload = FramePayloadReactionUpdate(decodeReactionUpdatePayload(r));
    case 43: payload = FramePayloadUserUpdated(decodeUserEntry(r));
    case 44: payload = FramePayloadChatDeleted(decodeChatDeletedPayload(r));
    case 45: payload = FramePayloadMemberUpdated(decodeMemberUpdatedPayload(r));
    case 48: payload = FramePayloadAck(r.remaining > 0 ? r.readBytes(r.remaining) : Uint8List(0));
    case 49: payload = FramePayloadError(decodeErrorPayload(r));
    case 64: payload = FramePayloadCreateChat(decodeCreateChatPayload(r));
    case 65: payload = FramePayloadUpdateChat(decodeUpdateChatPayload(r));
    case 66: payload = FramePayloadDeleteChat(decodeDeleteChatPayload(r));
    case 67: payload = FramePayloadGetChatInfo(decodeGetChatInfoPayload(r));
    case 68: payload = FramePayloadGetChatMembers(decodeGetChatMembersPayload(r));
    case 69: payload = FramePayloadInviteMembers(decodeInviteMembersPayload(r));
    case 70: payload = FramePayloadUpdateMember(decodeUpdateMemberPayload(r));
    case 71: payload = FramePayloadLeaveChat(decodeLeaveChatPayload(r));
    case 72: payload = FramePayloadMuteChat(decodeMuteChatPayload(r));
    case 73: payload = FramePayloadUnmuteChat(decodeUnmuteChatPayload(r));
    case 80: payload = FramePayloadGetUser(decodeGetUserPayload(r));
    case 81: payload = FramePayloadGetUsers(decodeGetUsersPayload(r));
    case 82: payload = FramePayloadUpdateProfile(decodeUpdateProfilePayload(r));
    case 83: payload = FramePayloadBlockUser(decodeBlockUserPayload(r));
    case 84: payload = FramePayloadUnblockUser(decodeUnblockUserPayload(r));
    case 85: payload = FramePayloadGetBlockList(decodeGetBlockListPayload(r));
    default: throw CodecError('unhandled FrameKind: ${header.kind.value}');
  }
