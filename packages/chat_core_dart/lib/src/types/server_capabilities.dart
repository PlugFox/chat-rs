...
extension type const ServerCapabilities(int value) implements int {
  ...
  static const List<ServerCapabilities> values = [
    mediaUpload,
    search,
    reactions,
    threads,
  ...
  ServerCapabilities add(ServerCapabilities flag) =>
      ServerCapabilities(value | flag.value);
  ServerCapabilities remove(ServerCapabilities flag) =>
      ServerCapabilities(value & ~flag.value);
  ServerCapabilities toggle(ServerCapabilities flag) =>
      ServerCapabilities(value ^ flag.value);
  bool get isEmpty => value == 0;
  bool get isNotEmpty => value != 0;
  ServerCapabilities operator ^(ServerCapabilities other) =>
      ServerCapabilities(value ^ other.value);
}
